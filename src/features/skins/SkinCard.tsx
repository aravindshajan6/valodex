 ...
 * The ★ sits beside the link (not inside it) so the card stays a single valid anchor.
 */
export function SkinCard({ skin, className, index, priority = false }: { skin: SkinCardData; className?: string; index?: number; priority?: boolean }) {
  ...
  return (
    <div
      ...
      className={cn(
        "group relative transition-[transform,box-shadow] duration-300 ease-out-expo hover:-translate-y-1 focus-within:-translate-y-1",
        "hover:shadow-[0_24px_60px_-24px_var(--tier)] focus-within:shadow-[0_24px_60px_-24px_var(--tier)]",
        className,
      )}
    >
      <Link href={skin.href} className="block outline-none">
        <div className="chamfer relative border border-line bg-ink-3 transition-colors duration-300 group-hover:border-(color:--tier) group-focus-within:border-(color:--tier)">
          ...
          <div className="relative aspect-[16/9] w-full">
            ...
            {skin.hasVideo && (
              <span className="absolute left-3 top-3 border border-holo/50 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-[0.2em] text-holo">Video</span>
            )}
          </div>
      ...
      <FavouriteButton kind="skins" id={skin.uuid} name={skin.name} className="absolute right-3 top-3 z-10" />
    </div>
  );
}

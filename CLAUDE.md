...
- Primitives in `@/components/ui`: `Container`, `SectionHeading`, `PageHero`, `Panel`, `Badge`, `Stat`.
- Motion in `@/components/motion`: wrap server markup in `<RevealGroup>`; **any `data-reveal` element outside a RevealGroup stays invisible** (CSS sets it to opacity 0 until JS reveals it), and `SectionHeading` uses `data-reveal` internally, so always wrap it (`PageHero` already does); mark children `data-reveal` to stagger-fade them in and `data-count="123"` to count numbers up. `<Magnetic>` for CTAs. For bespoke sequences, `import { animate, stagger, createTimeline } from "animejs"` inside a `"use client"` component's `useEffect`.
...
- Own only the paths assigned to you. Never edit `src/app/layout.tsx`, `globals.css`, `src/components/ui/*`, `src/components/motion/*`, `src/components/three/SceneCanvas.tsx|Scene3D.tsx`, `src/db/*`, `src/sync/*`, or another agent's paths. If you need a shared change, note it in your final report instead.
